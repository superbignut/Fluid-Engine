...
template <typename T> class ArrayAccessor<T, 1> final {
public:
  ArrayAccessor();
  ...
  template <typename Callback> void parallelForEach(Callback func) const;
...
template <typename T> using ArrayAccessor1 = ArrayAccessor1<T, 1>;

template <typename T> class ConstArrayAccessor<T, 1> {
public:
  ConstArrayAccessor();
